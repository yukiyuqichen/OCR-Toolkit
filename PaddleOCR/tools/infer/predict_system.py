...
def main(args, frame_ocr, process, process_end, file_num, count_num):
    process_end.set("Start!")
    frame_ocr.update()
    image_file_list = get_image_file_list(args.image_dir)
    image_file_list = image_file_list[args.process_id::args.total_process_num]
    text_sys = TextSystem(args)
    is_visualize = False     # 是否画图
    ...
    for idx, image_file in enumerate(image_file_list):
        ...
        for index, img in enumerate(imgs):
            ...
            print(res)
            for i in res:
                save_filename.append(os.path.basename(image_file))
                save_text.append(i["transcription"])
                save_score.append(i["score"])
            ...
            if is_visualize:
                ...
                logger.debug("The visualized image saved in {}".format(
                    os.path.join(draw_img_save_dir, os.path.basename(
                        save_file))))
        # 将结果存储为csv文件
        with open(os.path.join(draw_img_save_dir, "result.csv"), 'w', encoding='utf-8-sig') as f:
            f.write("filename,text,score" + "\n")
            for i in range(len(save_filename)):
                f.write(save_filename[i] + ',')
                f.write(save_text[i] + ',')
                f.write(str(save_score[i]) + '\n')
        # 打印进度
        count += 1
        count_num.set(count)
